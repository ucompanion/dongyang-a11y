...
(function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
          ...
          ga('create', 'UA-48993385-1', 'jnu.ac.kr');
          ga('send', 'pageview');
*/
...
//  - beforeSend 추가
function GetAjaxXml(url, data, success_function, loading, error_function, time) {
...
//			console.log("XMLDATA:" + xmlData);
			...
			if ($(xmlData).find('smartq-xml > result').text() != "0" && $(xmlData).find('smartq-xml > result').text() != "000") {
				...
				if (success_function != undefined && success_function != null) {
					...
					var width = (480*$('html').height())/$('html').width();
					$('#sb-site').css('min-height',width.toFixed(0)+'px');
				}
			}
		},
...
//	frm.appendChild(inputCurMenuId);
	...
	var store_id = $("#sapp_id");
	if (store_id != undefined && store_id != null && typeof store_id.val() == "string") {
		if(store_id.val()!=""){
			var inputMenuType = document.createElement("input");
			inputMenuType.setAttribute("type", "hidden");
			inputMenuType.setAttribute("name", "store_id");
			inputMenuType.setAttribute("value", store_id.val());
...
function fnConfirmDialog(title,msg,callback) {
...
//		 }
 ...
 function getStorage(name){
	 var rstValue = "";
//		 if(window.localStorage){
//			 rstValue = unescape(localStorage.getItem(name));
//		 }
//		 else{
		 rstValue = getCookie(name);
//		 }
	 return rstValue;
 }